...
local function calculateInertia2(device)
  local outputInertia
...
local function calculateTorqueLikeDiff(device) 
  print(device.inputAV)
  return (device.parent[device.parentOutputTorqueName] - ((device.friction + 0.01) * clamp(device.inputAV, -1, 1) + device.dynamicFriction * device.inputAV) * (device.wearFrictionCoef or 0) * (device.damageFrictionCoef or 0)) * device.gearRatio
end
...
local function updateTorqueWithoutClutch(device, dt)
  ...
  --for _, v in pairs(device.parent) do
  --  print(_ .. " -- " .. tostring(v))
  --end


  local diffTorqeu = calculateTorqueLikeDiff(device)

  device.outputTorque1 = diffTorqeu -- frictionTorque--(actualTorque - frictionTorque)
end
...
local function selectUpdates(device)
  ...
  device.torqueUpdate = updateTorqueWithoutClutch
end
...
local function new(jbeamData)
  local device = {
    ...
    onBreak = onBreak,
    validate = validate,
    calculateInertia = calculateInertia2,
    updateGFX = updateGFX,
